                                   ...
                                   WORD32 intra_pred_mode)
{
...

    {
        ...
        if ((log2_trafo_size > 2 && ps_sps->i1_chroma_format_idc != CHROMA_FMT_IDC_MONOCHROME) || ps_sps->i1_chroma_format_idc == 3)
        {
...
IHEVCD_ERROR_T  ihevcd_parse_sao(codec_t *ps_codec)
{
    ...
    else // if(!sao_merge_up_flag && !sao_merge_left_flag)
    {
        ...
        for(c_idx = 0; c_idx < (ps_sps->i1_chroma_format_idc != CHROMA_FMT_IDC_MONOCHROME ? 3: 1); c_idx++)
        {
